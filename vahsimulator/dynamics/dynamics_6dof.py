...
def diff_eq(state_vector: np.ndarray, loads_vector: np.ndarray, mpd: MassPropertiesData) -> np.ndarray:
    ...
    Jm = np.array([[ mpd.Ixx, -mpd.Ixy, -mpd.Ixz],
                   [-mpd.Ixy,  mpd.Iyy, -mpd.Iyz],
                   [-mpd.Ixz, -mpd.Iyz,  mpd.Izz]
                   ])

    # Getting inverve inertia matrix
    Jm_inv = np.linalg.inv(Jm)

    # Getting external forces represented in body frame
    F_B = loads_vector[0:3]

    # Getting external torques around vehicle's CG represented in body frame
    T_B = loads_vector[3:]
    ...
    omega_B_sm = skew_matrix(omega_B)
    ...
    # Rotational motion
    omega_B_dot = Jm_inv @ (T_B - omega_B_sm @ Jm @ omega_B) # Equation 1.3-18
...
class Dynamics6DOF(DynamicsBase):

    type: Literal["6DOF"] = "6DOF"
